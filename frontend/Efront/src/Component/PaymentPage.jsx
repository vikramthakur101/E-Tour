...
const PaymentPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    let formattedBookingDates = null;
    if (bookingDate) {
      const dateParts = bookingDate.split("/"); // Split "13/2/2025" into ["13", "2", "2025"]
      if (dateParts.length === 3) {
        const [day, month, year] = dateParts;
        formattedBookingDates = new Date(`${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`).toISOString();
      }
    }
  
    if (!formattedBookingDates) {
...
if (bookingDate) {
  const dateParts = bookingDate.split("/"); // ["13", "2", "2025"]
  if (dateParts.length === 3) {
    const [day, month, year] = dateParts;
...
console.log("Location state:", location.state);
    ...
    const paymentData = {
      ...
      tourname: tourName,
      email,
      bookingDate: formattedBookingDates.split("T")[0] // Sends only "YYYY-MM-DD"
